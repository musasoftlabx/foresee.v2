    ...
    <InputGroup className={cn("w-36", className)}>
      ...
      {!isValid && validationMessage && (
        <InputGroupAddon align="inline-end">
          <Tooltip>
            <TooltipTrigger children={<InputGroupButton size="icon-xs" />}>
...
const createOperatorsFromI18n = (i18n: FilterI18nConfig) => ({
  select: [
...
const getOperatorsForField = (field, i18n, values) => {
      ...
      <DropdownMenuContent align="start" className="w-fit min-w-fit">
        {operators.map((op: any) => (
          ...
          >
            <span>{op.label}</span>
            <CheckIcon
              className={`${cn("text-primary ms-auto", op.value === operator ? "opacity-100" : "opacity-0")}`}
            />
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
...
export const FiltersContent = ({ filters, fields, onChange }: any) => {
  const context = useFilterContext();
  ...
  const removeFilter = useCallback(
    (filterId: string) => {
      onChange(filters.filter((filter: any) => filter.id !== filterId));
    },
    [filters, onChange],
  );
...
export function Filters({
  ...
  const updateFilter = useCallback(
    (filterId: string, updates: Partial<Filter<any>>) => {
      onChange(
        filters.map((filter: any) => {
          if (filter.id === filterId) {
            const updatedFilter = { ...filter, ...updates };
    ...
    >
      ...
      >
        {selectableFields.length > 0 && (
          ...
          >
            <DropdownMenuTrigger children={triggerButton} />
            <DropdownMenuContent
